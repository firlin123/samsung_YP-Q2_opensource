...
/* $Id: SDL_ttf.c,v 1.10 2008/12/12 07:45:24 n.y.lee Exp $ */
...
/* By leeth, in case of NULL chars, SDL TTF is returned NULL Pointer.
   But, AppMain receive NULL pointer, and then be shutdowned.
   for preventing this condition, if NULL char is occured, we will exchange
   SPACE chars at 20070102 */
#define NULL_CHAR		0x23
...
#if 1 /* By leeth, for display full glyph, base line is increaed 1 line at 20070102 */
	  font->height	= font->ascent - font->descent + /* baseline */ 3;
#else
...
static FT_Error Load_Glyph( TTF_Font* font, Uint16 ch, c_glyph* cached, int want )
{
	...
	if ( ((want & CACHED_BITMAP) && !(cached->stored & CACHED_BITMAP)) ||
		...
		if( error && ch != 0x20 ) 
		/* !!FIXME!!. space is not rendered on some fonts */ 
		{
			return error;
		}
