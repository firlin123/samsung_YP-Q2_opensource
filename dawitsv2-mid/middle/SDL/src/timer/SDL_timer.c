...
long  SDL_ResetTimerBase(void)
{
	int rtcfd;		/* file descriptor rtc driver */

	/* 2008.12.23: For compensating the delta value between RTC and xtime {  */
	/* Since, the xtime does not be updated in sleep mode */
	rtcfd = open("/dev/rtc", O_RDWR);
	if(rtcfd >= 0) {
		ioctl(rtcfd, 34);
	}
	close(rtcfd);
