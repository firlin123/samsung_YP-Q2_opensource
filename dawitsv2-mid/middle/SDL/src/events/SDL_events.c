...
/* 2009.01.05: set restirction to key buffer */
#define SDL_KEY_QUEUE_MAX 6
		...
		if ( action == SDL_ADDEVENT ) {
			for ( i=0; i<numevents; ++i ) {
				...
				if(events[i].type == SDL_KEYDOWN || 
				    events[i].type == SDL_KEYUP) {
					if(key_queue_count >= SDL_KEY_QUEUE_MAX 
						|| SDL_keyenable == 0) {
						; /* do nothing */
					} else {
			...
			while ((used < numevents)&&(spot != SDL_EventQ.tail)) {
				if ( mask & SDL_EVENTMASK(SDL_EventQ.event[spot].type) ) {
					...
					if ( action == SDL_GETEVENT ) {
						/* 2009.01.05: set restirction to key buffer */
						if(SDL_EventQ.event[spot].type == SDL_KEYDOWN || 
						    SDL_EventQ.event[spot].type == SDL_KEYUP) {
...
/* Clear key buffer form SDL_event */
void SDL_ClearKeyEvent(void)
{
	int spot;
	...
	if (SDL_mutexP(SDL_EventQ.lock) == 0 ) {
		...
		while (spot != SDL_EventQ.tail) {
			if(SDL_EventQ.event[spot].type == SDL_KEYDOWN || 
			    SDL_EventQ.event[spot].type == SDL_KEYUP) {
				printf("[SDL_ClearKeyEvent] cut %dth key remain = %d\n", spot, key_queue_count);
...
/* Disable key event. */
void SDL_DisableKeyEvent(void)
{
	SDL_keyenable = 0;
}
/* Enable key event. */
void SDL_EnableKeyEvent(void)
{
	SDL_keyenable = 1;
}
