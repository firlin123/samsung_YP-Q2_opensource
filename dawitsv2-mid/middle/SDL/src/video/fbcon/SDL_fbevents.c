...
int FB_OpenMouse(_THIS)
{
	...
	/* A16 TOUCHSCREEN SUPPORT */
	if ( mousedrv && (SDL_strcmp(mousedrv, "A16TS") == 0) ) {
		mouse_fd = open(mousedev, O_RDWR);
		if ( mouse_fd >= 0 ) {
#ifdef DEBUG_MOUSE
			fprintf(stderr, "Using A16 touchscreen\n");
#endif
			mouse_drv = MOUSE_A16TS;
		}
		else if ( mouse_fd < 0 ) {
			mouse_drv = MOUSE_NONE;
		}
...
static void handle_mouse(_THIS)
{
	...
	static unsigned short prev_pos[2];  // A16TS
	unsigned short *cur_pos = (unsigned short *)mousebuf; // A16TS
	...
	switch (mouse_drv) {
		...
		case MOUSE_BM:
			...
			break;
	        case MOUSE_A16TS: 
			nread = read(mouse_fd, mousebuf, 4); 
			if ( cur_pos[0] < 100 && cur_pos[1] < 100 ) { 
				button = 0; // release 
			} else if ( prev_pos[0] < 100 && prev_pos[1] < 100 ) { 
				button = 0x04; // pressed 
			}

			FB_vgamousecallback(0, 0, (2500-cur_pos[0])*128/2500, cur_pos[1]*160/2500);

#if 0 // def DEBUG_MOUSE
			fprintf(stderr, "%s: orig: (%d,%d), translated(%d,%d)\n", __FUNCTION__, 
				cur_pos[0], cur_pos[1], (2500-cur_pos[0])*128/2500, cur_pos[1]*160/2500);
#endif 
...
void FB_PumpEvents(_THIS)
{
	...
	do {
		...
		if ( keyboard_fd >= 0 ) {
			rfd[num_fds].fd = keyboard_fd; 
			rfd[num_fds].events = POLLIN; 
			num_fds++;
		}
		if ( mouse_fd >= 0 ) {
			rfd[num_fds].fd = mouse_fd; 
			rfd[num_fds].events = POLLIN; 
			num_fds++;
		}

		if (status = poll(rfd, num_fds, 10 ) < 0 ) { 
			perror("poll error"); 
			return; 
		}

		for ( i = 0; i < num_fds; i++ ) { 
			if ( rfd[i].revents & rfd[i].events ) { 
				/* received a requested event for that fd */ 
				if( rfd[i].fd  == keyboard_fd ) 
					handle_keyboard(this);
				else if ( rfd[i].fd  == mouse_fd ) 
					handle_mouse(this); 
				else 
