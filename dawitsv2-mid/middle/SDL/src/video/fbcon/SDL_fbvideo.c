...
static int FB_VideoInit(_THIS, SDL_PixelFormat *vformat)
{
	...
	fprintf(stderr, "HEECHUL: open %s successfun\n", SDL_fbdev); 
	...
	fprintf(stderr, "HEECHUL: get video hardware info\n"); 
...
#ifdef FBCON_DEBUG
	fprintf(stderr, "HEECHUL: open keyboard\n"); 
#endif 
...
#if HW_A16
#include "framebuffer_ioctl.h"

static void FB_DirectUpdate(_THIS, int numrects, SDL_Rect *rects)
{
	struct fb_area rp_area;

	rp_area.x = (int)rects->x;
	rp_area.y = (int)rects->y;
	rp_area.w = (int)rects->w;
	rp_area.h = (int)rects->h;
	
	ioctl(console_fd, FBIO_STMP36XX_FB_FLUSH, &rp_area);
}
...
/* sgtl 
	: for video player osd control
*/
int FB_GetConsoleFd(SDL_VideoDevice *this)
{
	return console_fd;
}
