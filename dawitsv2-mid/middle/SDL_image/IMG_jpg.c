...
#define THUMBNAIL_WIDTH	64 /*Q2*/		//46 /*S3*/
#define VIEW_WIDTH			320 /* Q2 */		//220 /*S3*/
#define ZOOM_200_WIDTH	(VIEW_WIDTH * 2)
#define ZOOM_400_WIDTH	(VIEW_WIDTH * 4)
...
SDL_Surface *IMG_LoadJPG_RW(SDL_RWops *src)
{
	...
	/* A16.tknoh.070211-1.Remove Progressive Jpeg support */
	if (cinfo.progressive_mode == TRUE)
	{
		lib.jpeg_destroy_decompress(&cinfo);
		SDL_RWseek(src, start, SEEK_SET);
		IMG_QuitJPG();
		//printf("This is a Progressive Jpeg file!!-destroy decompress\n"); 
		return surface;
	}
	/* TODO(tknoh) : Q2 Jpeg Spec - 5MB юлго */
	#if 0
	if ((cinfo.image_width > 3000) || (cinfo.image_height > 3000))
	{
		lib.jpeg_destroy_decompress(&cinfo);
		SDL_RWseek(src, start, SEEK_SET);
		IMG_QuitJPG();
		printf("This Jpeg size is over spec (3000*3000)!!-destroy decompress\n");
		return surface;
	}
	#endif
	...
	if(cinfo.num_components == 4) {
...
#ifdef SCALE_MODIFY
		// Scale Decoding Debug
		//printf("---\n");
		//printf("(%d,%d)", cinfo.image_width, cinfo.image_height);
	#ifdef SCALE_DENOM
		/* Calculate denom value */
		//int min_val = MIN(cinfo.image_width, cinfo.image_height);
		int width = cinfo.image_width;
		int denom;
		
		char* pic_env = getenv("PIC_STATE");

		if(pic_env == NULL)
		{
			//printf("[SDL_Image]PIC_STATE = NULL\n");
			denom = 1;
		}
		else if(strcmp(pic_env, "THUMBNAIL") == 0)
		{
			if((width / 8) > THUMBNAIL_WIDTH)			
			{
				denom = 8;
			}
			else if((width / 4) > THUMBNAIL_WIDTH)			
			{
				denom = 4;
			}
			else if((width / 2) > THUMBNAIL_WIDTH)			
			{
				denom = 2;
			}
			...
			//printf("[SDL_Image]THUMBNAIL(1/%d)\n", denom);						
		}
		else if(strcmp(pic_env, "VIEW") == 0)
		{
			if((width / 8) > VIEW_WIDTH)			
			{
				denom = 8;
			}
			else if((width / 4) > VIEW_WIDTH)			
			{
				denom = 4;
			}
			else if((width / 2) > VIEW_WIDTH)			
			{
				denom = 2;
			}
			...
			//printf("[SDL_Image]VIEW(1/%d)\n", denom);						
		}
		else if(strcmp(pic_env, "ZOOM_200") == 0)
		{
			if((width / 8) > ZOOM_200_WIDTH)			
			{
				denom = 8;
			}
			else if((width / 4) > ZOOM_200_WIDTH)			
			{
				denom = 4;
			}
			else if((width / 2) > ZOOM_200_WIDTH)			
			{
				denom = 2;
			}
			else if((width) > ZOOM_200_WIDTH)			
			{
				denom = 1;
			}
			else
			...
			//printf("[SDL_Image]ZOOM_200(1/%d)\n", denom);						
		}
		else if(strcmp(pic_env, "ZOOM_400") == 0)
		{
			if((width / 8) > ZOOM_400_WIDTH)			
			{
				denom = 8;
			}
			else if((width / 4) > ZOOM_400_WIDTH)			
			{
				denom = 4;
			}
			else if((width / 2) > ZOOM_400_WIDTH)			
			{
				denom = 2;
			}
			else if((width) > ZOOM_400_WIDTH)			
			{
				denom = 1;
			}
			else
			...
			//printf("[SDL_Image]ZOOM_400(1/%d)\n", denom);						
		}
		else
		{
			denom = 1;
			//printf("[SDL_Image]else(1/1)\n");									
		}
		...
		cinfo.scale_num 	= 1;
		cinfo.scale_denom	= 1;
	#endif //#ifdef SCALE_DENOM

#endif //#ifdef SCALE_MODIFY
