...
#ifdef STMP_BUILD
extern void SetVideoFrameBuffer(unsigned char *frame_buffer, int fb_size);
#endif
		...
		uint32_t format)
{
...
#endif
	{
		...
		/* added by jinho.lim@samsung.com */
		#ifdef STMP_BUILD
		SetVideoFrameBuffer(frame_buffer, fb_size);

		/* for playback of under 320*240 size */
		fb_mirror mirror;
		mirror.enable = 0;
		mp_msg(MSGT_VO, MSGL_DBG2, "===> mirror.enable=%d\n", mirror.enable);
		ioctl(fb_dev_fd, FBIO_FB_MIRROR_VIDEO, &mirror);
		#endif
...
{
#ifdef STMP_BUILD 
	ioctl(fb_dev_fd, STMP37XXFB_IOCT_REFRESH);
#endif
}
