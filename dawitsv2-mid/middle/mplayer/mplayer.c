...
static int fill_audio_out_buffers(void)
{
	...
	//printf("===> freebytes=%d, ao_data.outburst=%d\n", bytes_to_write, ao_data.outburst);
...
/* added by jinho.lim@samsung.com */
#ifdef STMP_BUILD
int WriteToMDSrv(audio_fd, data, len)
{
	len=write(audio_fd, data, len);
}


int GetDSPDevice(void)
{
	/* modified by jinho.lim */
	return open("dev/dsp", O_RDWR);
}
#endif
...
int gui_no_filename=0;
  ...
  if ( argv[0] )
  {
	...
	/* external cfg is not supported for STMP_BUILD */
#ifndef STMP_BUILD
...
/*========================== PLAY VIDEO ============================*/
  ...
  //printf("===> play video !!!, mpctx->num_buffered_frames=%d\n", mpctx->num_buffered_frames);
  ...
  if (!mpctx->num_buffered_frames) {
	  ...
	  //printf("===> update video ÈÄ !! mpctx->num_buffered_frames=%d, blit_frame=%d?n", mpctx->num_buffered_frames, blit_frame);	  
...
//====================== FLIP PAGE (VIDEO BLT): =========================
	   ...
	   mpctx->num_buffered_frames--;
	   //printf("===> Flip page ÈÄ !! mpctx->num_buffered_frames=%d\n", mpctx->num_buffered_frames);	   
...
if(step_sec>0) {
 ...
 printf("===> step_sec=%d, rel_seek_secs=%d\n", step_sec, rel_seek_secs);
 }
...
if(rel_seek_secs || abs_seek_pos){
	printf("===> rel_seek_secs=%.2lf, abs_seek_pos=%.2lf\n", rel_seek_secs, abs_seek_pos);
