...
//        sh_video->bih=malloc(chunksize); memset(sh_video->bih,0,chunksize);
  ...
  if (pos >= 0) {
	  ...
	  #ifdef STMP_BUILD
	  //
	  // mplayer bug fix, preroll is msec, but duration is nsec
	  // 
      asf->movielength=(fileh->play_duration-fileh->preroll*10000)/10000000LL;
	  asf->preroll = fileh->preroll;
	  asf->packet_num = fileh->num_packets;
	  demuxer->packet_nums =fileh->num_packets;	  
	  #else
