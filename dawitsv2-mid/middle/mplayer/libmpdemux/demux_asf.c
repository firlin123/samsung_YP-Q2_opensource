  ...
  if(ds){
    ...
    { demux_packet_t* dp;
      ...
      if (asf->asf_is_dvr_ms)
        dp->pts=(time-asf->preroll)*0.0000001;
      else
        dp->pts=(time-asf->preroll)*0.001;
#else
	  if (asf->asf_is_dvr_ms)
...
#endif
      dp->flags=keyframe;
      //if(ds==demux->video) printf("ASF time: %8d  dp->pts: %f  \n",time, dp->pts);
      dp->pos=demux->filepos;
      ds->asf_packet=dp;
      ds->asf_seq=seq;
      // we are ready now.
...
static int demux_asf_fill_buffer(demuxer_t *demux, demux_stream_t *ds){
  struct asf_priv* asf = demux->priv;
	...
	demux->invalid_frame = 0;

	//printf("=========> 111 : demux_asf_fill_buffer, demux->pts=%.2f\n", demux->video->pts);
    ...
    
    {
	    ...
	    if(((flags>>5)&3)!=0){
              ...
              if(plen>asf->packetsize) 
			  {
				...
				/* added by jinho.lim - invalid segment */
				printf("=====================> invalid frame !!!\n");
				demux->invalid_frame = 1;
				return 0;
              }
	    } else {
		...
		while(len>0){
		  ...
		  ++seq;
			...
			//printf("=========> 444 : demux_asf_fill_buffer, demux->pts=%.2f\n", demux->video->pts);
            return 1; // success
    }
...
//    sh_video_t *sh_video=d_video->sh;
  ...
  //================= seek in ASF ==========================
	...
	printf("\r -- asf: after seek pos=%d, v_pts=%.2f, newpos=%d -- \n", demuxer->filepos, d_video->pts, newpos);
...
#else
    //float p_rate=asf->packetrate; // packets / sec
	double total_time = demuxer_get_time_length(demuxer);
	float p_rate = (double)asf->packet_num/total_time;
	
	int i;
	unsigned long skip_bytes=asf->packetsize*p_rate;
	long d_ff;
	
    off_t rel_seek_packs;
    off_t rel_seek_bytes;
    off_t newpos;
	float seek_sec = rel_seek_secs;

	/* for around last frame */
	if(seek_sec >= total_time-3)
	{
		rel_seek_packs = (seek_sec*p_rate);
		...
		//printf("ASF_seek: 111 %.2f, asf->packetsize=%d, rel_seek_bytes=%d, p_rate=%.2f, newpos=%d \n",
		//   seek_sec, asf->packetsize, rel_seek_bytes, p_rate, newpos);
		stream_seek(demuxer->stream, newpos);
		
		if (asf->asf_is_dvr_ms) asf->dvr_last_vid_pts = 0.0f;
		if (d_video->id >= 0)
			if(!ds_fill_buffer(d_video))return; // skip frame.  EOF?
		
		//printf("\r -- asf: 111 after seek pos=%d, v_pts=%.2f, newpos=%d -- \n", demuxer->filepos, d_video->pts, newpos);
		if(sh_audio){
		  if(!ds_fill_buffer(d_audio)) return;
		}
	}
	else
	{
		...
		for(i=0;i<2;i++)
		{
		    ...
		    //printf("ASF_seek: %.2f, asf->packetsize=%d, rel_seek_bytes=%d, p_rate=%.2f, newpos=%d \n",
		    //   seek_sec, asf->packetsize, rel_seek_bytes, p_rate, newpos);
			stream_seek(demuxer->stream, newpos);
		
			if (asf->asf_is_dvr_ms) asf->dvr_last_vid_pts = 0.0f;
			if (d_video->id >= 0)
				if(!ds_fill_buffer(d_video)) return;
		
			//printf("\r -- asf: after seek pos=%d, v_pts=%.2f, newpos=%d -- \n", demuxer->filepos, d_video->pts, newpos);
			if((long)d_video->pts == (long)rel_seek_secs ||
				(long)d_video->pts == (long)rel_seek_secs-1 ||(long)d_video->pts == (long)rel_seek_secs+1 ||
				(long)d_video->pts == (long)rel_seek_secs-2 ||(long)d_video->pts == (long)rel_seek_secs+2 ||
				(long)d_video->pts == (long)rel_seek_secs-3 ||(long)d_video->pts == (long)rel_seek_secs+3 ||
				(long)d_video->pts == (long)rel_seek_secs-4 ||(long)d_video->pts == (long)rel_seek_secs+4 ||
				(long)d_video->pts == (long)rel_seek_secs-5 ||(long)d_video->pts == (long)rel_seek_secs+5)
			{
				//printf("===> First +-5sec find find find !!!\n");
				break;
			}
			
			if((long)d_video->pts > (long)rel_seek_secs)
			{
				d_ff = (long)d_video->pts - (long)rel_seek_secs;
				...
				if(seek_sec - d_ff <= 0)
				{
					...
					seek_sec -= d_ff;
				//printf("+++ d_ff=%d, seek_sec=%.2f\n", d_ff, seek_sec);
				//if(seek_sec <= 0) break;			
			}
			else
			{
				d_ff = (long)rel_seek_secs - (long)d_video->pts;

				if(seek_sec + d_ff >= total_time)
				{
					//printf("==================> diff over total time !!!\n");
					seek_sec += (total_time-seek_sec)/2;
				}
				else
					seek_sec += d_ff;
				//printf("--- d_ff=%d, seek_sec=%.2f\n", d_ff, seek_sec);
				//if(seek_sec >= total_time-1) break;
			}
		}
		...
		//while(1)
		{
			...
			//printf("\r -- asf: after seek pos=%d, v_pts=%.2f, newpos=%d -- \n", demuxer->filepos, d_video->pts, newpos);
			if((long)d_video->pts == (long)rel_seek_secs ||
				(long)d_video->pts == (long)rel_seek_secs-1 ||
				(long)d_video->pts == (long)rel_seek_secs+1)
			{
				//printf("===> find find find !!!\n");
				break;
			}
			
			if((long)d_video->pts > (long)rel_seek_secs)
			{
				if((newpos - skip_bytes) > 0) 
					newpos -= asf->packetsize;
				else newpos = 0;
			}
			else
				newpos += asf->packetsize;
...
#ifdef STMP_BUILD
	/* added by jinho.lim@samsung.com : memory leak problem */
	if(asf->packet)
		free(asf->packet);
#endif
