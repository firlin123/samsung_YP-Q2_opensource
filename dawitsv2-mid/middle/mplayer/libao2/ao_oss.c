...
/* added by jinho.lim */
  ...
  // Open Output File
  printf("===> fileout=%s\n", "out.pcm");
  fppcm = fopen("/mnt/usb/out.pcm","wb");
#endif
...
#ifdef STMP_BUILD
  audio_fd = GetDSPDevice();
#else
   audio_fd=open(dsp, O_RDWR);
#endif
#else
...
ac3_retry:  
  ...
  if(format != AF_FORMAT_AC3) {
	...
	return 0;
	  	...
	  	/* by jinho.lim to support channel 2 filter */
		ao_data.channels = 2;	
...
static int play(void* data,int len,int flags){
	...
	/* to apply audio effect and write - jinho.lim */
#ifdef STMP_BUILD
    len = WriteToMDSrv(audio_fd, data, len);
	//len = write(audio_fd, data, len);
	if (len == -1) {
		/* The write failed to make it down to the driver. */
		printf("ERROR: Mplayer dsp write failed \n");
	}	
#else
	len=write(audio_fd, data, len);
#endif
	//fwrite(data, len, 1, fppcm);
    return len;
}
