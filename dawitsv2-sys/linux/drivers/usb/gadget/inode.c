...
struct ep_data {
	...
	struct dentry			*dentry;
	struct inode			*inode;
#ifdef CONFIG_USB_STMP37XX
	bool bCancelled;
#endif
};

#ifdef CONFIG_USB_STMP37XX
#undef USE_SAMSUNG_AIO
#define USE_SAMSUNG_OCRAM
#ifndef USE_SAMSUNG_OCRAM
#undef USE_SAMSUNG_AIO
#endif
#ifdef USE_SAMSUNG_OCRAM
#include <asm/arch-stmp37xx/ocram.h>
#endif
#ifdef USE_SAMSUNG_AIO

#include <linux/syscalls.h>

/* AREAD api definition */ 
static void aread_init(struct usb_ep *ep); 

static wait_queue_head_t aread_wait_queue;
static wait_queue_head_t pwrite_wait_queue;

static volatile struct gadgetfs_aread_ioctl  cur_aread; 
static volatile int aread_nrequest  = 0;  /* increased by AREAD ioctl. decreased by ISR */ 
static volatile int aread_ncomplete = 0;  /* increased by ISR. decreased by poll */ 
static int no_overlap = 0;  // to collect overlapping information

static int aread_req_cnt = 0;
struct usb_request *aread_req[3];
#endif
#endif
...
static void aread_init (struct usb_ep *ep)
{
	...
	init_waitqueue_head(&aread_wait_queue);
	init_waitqueue_head(&pwrite_wait_queue);
	
	for (i = 0; i < ARRAY_SIZE(aread_req); i++) {
		if (!aread_req[i])
			aread_req[i] = usb_ep_alloc_request(ep, GFP_ATOMIC);
	}
}

static void ep_aread_complete (struct usb_ep *ep, struct usb_request *req)
{
	...
	VDEBUG(epdata->dev, "\n@@done %p actual-%d status-%d\n", req->buf, req->actual, req->status);

	if ( aread_nrequest ) {	/* there's pending AREAD request */ 
		aread_nrequest --;
		aread_ncomplete++; 
		wake_up_interruptible(&pwrite_wait_queue); // protect nreq go over 2 
		wake_up_interruptible(&aread_wait_queue); 
...
#ifdef CONFIG_USB_STMP37XX
		if (epdata->bCancelled) {
			epdata->status = -EINVAL;
		}
#endif
...
#ifdef CONFIG_USB_STMP37XX
#ifdef USE_SAMSUNG_OCRAM
	kbuf = buf_shared ? get_shared_virt(buf) : kmalloc (len, GFP_KERNEL);
#else
...
#ifdef CONFIG_USB_STMP37XX
#ifdef USE_SAMSUNG_OCRAM
	if (!buf_shared && value >= 0 && copy_to_user (buf, kbuf, value))
		value = -EFAULT;
#else
		...
		unsigned code, unsigned long value)
{
...
#ifdef CONFIG_USB_STMP37XX
#ifdef USE_SAMSUNG_AIO
	struct gadgetfs_aread_ioctl  aread; 
	void *kbuf;
	int i;
	struct usb_request *req;
#endif

	if (code != GADGETFS_CANCEL && code != GADGETFS_FIFO_FLUSH) {
#endif
	...
	if (likely (data->ep != NULL)) {
		...
		case GADGETFS_CANCEL:
			...
			status = usb_ep_dequeue (data->ep, data->req);
#ifdef USE_SAMSUNG_AIO
			for (i = 0; i < ARRAY_SIZE(aread_req); i++)
				status = usb_ep_dequeue (data->ep, aread_req[i]);
#endif
			break;

#ifdef USE_SAMSUNG_AIO
		case GADGETFS_AREAD: /* asynchronous USB read */ 
			copy_from_user(&aread, (char *)value, sizeof(struct gadgetfs_aread_ioctl));

			if (unlikely(cur_aread.buf == aread.buf)) {
				printk("GADGETFS_AREAD: buffer overlapped \n"); 
				printk("prev - buf(0x%x), size(%d)\n", (unsigned)cur_aread.buf, cur_aread.size);
				printk("curr - buf(0x%x), size(%d)\n", (unsigned)aread.buf, aread.size);
				status = -EINVAL;
				break;
			}
			spin_unlock_irq(&data->dev->lock ); 

			status = wait_event_interruptible(pwrite_wait_queue, 
											(aread_nrequest < 2) ||
											data->bCancelled || 
											//data->dev->state == STATE_DEV_RESET || 
											data->dev->state == STATE_DEV_UNCONNECTED );

			spin_lock_irq(&data->dev->lock);

			if ( status < 0 ) {
				printk("%s: AREAD is interrupted. rbuf(0x%0x), kbuf(0x%x), size(%d)\n", 
					   __FUNCTION__, aread.wfd, (unsigned)aread.buf, aread.size);
				break;
			}

			if ( data->bCancelled ) {
				printk("GADGETFS_AREAD: this is canceled\n"); 
				printk("current nreq(%d), ncompl(%d)\n", 
				       aread_nrequest, aread_ncomplete); 
				status = -EINVAL; 
				break;
			}

			if ( /*data->dev->state == STATE_RESET || */data->dev->state == STATE_DEV_UNCONNECTED) 
			{
				printk("GADGETFS_AREAD: device is aborted (%s)\n", 
				       //(data->dev->state == STATE_RESET) ? "reset" : 
					   "unconnected" ); 
				printk("current nreq(%d), ncompl(%d)\n", 
				       aread_nrequest, aread_ncomplete); 
				status = -EINVAL; 
			...
			if ( !is_shared(aread.buf) ) { 
				printk("%s: Not shared buffer\n", __FUNCTION__); 
				status = -EINVAL; 
				break;
			}

			cur_aread.buf = aread.buf; 
			cur_aread.size = aread.size; 
			
			kbuf = get_shared_virt(aread.buf);

			req = aread_req[aread_req_cnt];
			if (++aread_req_cnt >= 3) {
				aread_req_cnt = 0;
			}

			//printk("%s: AREAD: rbuf(0x%0x), kbuf(0x%x), size(%d)\n", 
			//       __FUNCTION__, (unsigned)aread.buf, (unsigned)kbuf, aread.size);

			aread_nrequest ++;

			req->buf = kbuf;
			req->length = aread.size;
			req->complete = ep_aread_complete;
			req->context = NULL;
			value = usb_ep_queue(data->ep, req, GFP_ATOMIC);
			if (unlikely(0 != value))
				status = -EINVAL; 
			break;

		case GADGETFS_PWRITE: /* poll + write */ 
			copy_from_user(&aread,(char *)value,sizeof(struct gadgetfs_aread_ioctl));
			...
			if ( aread_ncomplete >= 2 ) 
				no_overlap ++; 

			spin_unlock_irq(&data->dev->lock);
			
			status = wait_event_interruptible(aread_wait_queue, 
						 (aread_ncomplete > 0) ||
 						  data->bCancelled || 
						  //data->dev->state == STATE_RESET || 
						  data->dev->state == STATE_DEV_UNCONNECTED );

			spin_lock_irq(&data->dev->lock);  

			if ( status < 0 ) {
				printk("%s: PWRITE is interrupted. wfd(%d), buf(0x%0x), size(%d)\n", __FUNCTION__, aread.wfd, (unsigned)aread.buf, aread.size);
				break;
			}

			if ( data->bCancelled ) {
				printk("GADGETFS_PWRITE: this is canceled\n"); 
				printk("current nreq(%d), ncompl(%d)\n", 
				       aread_nrequest, aread_ncomplete); 
				status = -EINVAL; 
				break;
			...
			if ( /*data->dev->state == STATE_RESET || */data->dev->state == STATE_DEV_UNCONNECTED) 
			{
				printk("GADGETFS_PWRITE: device is aborted (%s)\n", 
				       //(data->dev->state == STATE_RESET) ? "reset" : 
					   "unconnected" ); 
				printk("current nreq(%d), ncompl(%d)\n", 
				       aread_nrequest, aread_ncomplete); 
			...
			if ( unlikely(cur_aread.buf == aread.buf )) {
				printk("GADGETFS_PWRITE: read buffer and write buffer is same\n"); 
				printk("rbuf(0x%x), size(%d)\n", (unsigned)cur_aread.buf, cur_aread.size);
				printk("wbuf(0x%x), size(%d)\n", (unsigned)aread.buf, aread.size);
				status = -EINVAL;   
				break; 
			} 

			aread_ncomplete --; // consume completion 

			//printk("%s: PWRITE: wfd(%d), buf(0x%0x), size(%d)\n", 
			//      __FUNCTION__, aread.wfd, (unsigned)aread.buf, aread.size); 
			spin_unlock_irq(&data->dev->lock);

			status = sys_write(aread.wfd, aread.buf, aread.size); 
			if ( status < 0 ) {
				printk("%s: Write failed fd(%d) buf(0x%x) size(%d)\n", 
				       __FUNCTION__, aread.wfd, (unsigned)aread.buf, aread.size ); 
			} 
			break;
#endif
#endif /* CONFIG_USB_STMP37XX */
...
#ifdef CONFIG_USB_STMP37XX
#ifdef USE_SAMSUNG_OCRAM
	if (!is_shared(priv->iv[0].iov_base)) {
#endif
#endif
...
#ifdef CONFIG_USB_STMP37XX
#ifdef USE_SAMSUNG_OCRAM
	buf = is_shared(iov->iov_base) ? get_shared_virt(iov->iov_base) : kmalloc(iocb->ki_left, GFP_KERNEL);
#else
